...
The final element of the architecture is a powerful query engine (Photon) which lets the data consumers create aggregations and integration efficiently for themselves. Here, ad-hoc queries can be written and executed, an analytics-specific schema (Snowflake, Star, etc.) can be applied, tables can be joined, unioned, grouped, etc. It is important to perform all the necessary transformations in this layer because the other self-service alternatives (Excel, Power Query, Qlik Load Editor, etc.) are almost always terrible performance-wise and also hard to use for complex transformations. These transformations are not necessarily re-calculated every time when a user executes a query as caching query results is supported by Databricks SQL.
