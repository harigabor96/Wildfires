...
object PipelineRooter {

  def execute(spark: SparkSession, conf: Conf): Unit = conf.pipeline() match {
    ...
    case _ => throw new Exception("Pipeline is not registered in the rooter!")
  }
